...
const width = sch.Width, height = sch.Height, length = sch.Length;
const paletteArr = [];
for(const k of Object.keys(palette)) paletteArr[palette[k]] = k;
const base64 = Buffer.from(dataBuf).toString('base64');
const out = { width, height, length, palette: paletteArr, data: base64 };
fs.writeFileSync('bastion_treasure_data.js', 'const BASTION_TREASURE_SCHEM = ' + JSON.stringify(out) + ';\n');
console.log('ok', width, height, length, paletteArr.length, base64.length);
